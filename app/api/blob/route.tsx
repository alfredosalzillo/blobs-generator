...
const formatPoints = (points: { x: number; y: number }[], closed = false) => {
  ...
  if (closed) {
    const t = n[n.length - 1], o = n[n.length - 2], a = n[0], f = n[1];
...
const spline = (points: { x: number; y: number }[] = [], tension = 1, closed = false) => {
  const t = formatPoints(points, closed), o = t.length, a = o - 4, f = closed ? t[2] : t[0], u = closed ? t[3] : t[1];
  let l = `M${[f, u]}`;
  const x = closed ? 2 : 0, P = closed ? o - 4 : o - 2, y = 2;
  for (let s = x; s < P; s += y) {
    const d = s ? t[s - 2] : t[0], g = s ? t[s - 1] : t[1], p = t[s], h = t[s + 1], i = t[s + 2], r = t[s + 3], m = s !== a ? t[s + 4] : i, I = s !== a ? t[s + 5] : r, $ = p + (i - d) / 6 * tension, z = h + (r - g) / 6 * tension, C = i - (m - p) / 6 * tension, L = r - (I - h) / 6 * tension;
    l += `C${[$, z, C, L, i, r]}`;
  }
  return l;
};
